...
skim(crime)
```
## Load and Typeset the test data (down).
```{r,warning=FALSE,message=FALSE}
# crime_test <- read_csv("./data/crime_test.csv")
# crime_test
# 
# crime_test <-   crime_test %>% mutate(
#     highest_offense_description = as.factor(highest_offense_description),
#     highest_offense_code           = as.factor(highest_offense_code),
#     family_violence                = as.factor(family_violence),
#     occurred_date_time_month       = as.ordered(occurred_date_time_month),
...
## Extract the appropriate test data (down). 
```{r,warning=FALSE,message=FALSE}
# library(tidyverse)
# set.seed(1906525)
# 
# # Bin highest_offense_code: Top 20 frequent codes, others as "Other"
# top_codes <- crime_test %>% count(highest_offense_code, sort=TRUE) %>% slice_head(n=20) %>% pull(highest_offense_code)
...
# 
# crime_sample_50pct <- crime_test %>%
#   group_by(hoc_binned, occurred_date_time_year, family_violence, apd_sector) %>%
#   sample_frac(0.50) %>%
#   ungroup()
# 
# crime_sample_50pct <- crime_sample_50pct %>% mutate(
#   hoc_binned = NULL
# )
# 
# crime_sample_50pct <- crime_sample_50pct %>% mutate(
...
#   )
# 
# #### write_csv(crime_sample_50pct, "./data/crime_test_updated.csv") # use if needed

```

##Load new test data and see.
```{r,warning=FALSE,message=FALSE}
# crime_tu <- read_csv("./data/crime_test_updated.csv")
# skim(crime_tu)
```
